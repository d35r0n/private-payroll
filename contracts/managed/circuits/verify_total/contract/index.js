...
export class Contract {
  ...
  constructor(...args_0) {
    ...
    this.circuits = {
      verify_total(context, ...args_1) {
        return { result: pureCircuits.verify_total(...args_1), context };
      }
    };
    this.impureCircuits = {};
  ...
  _verify_total_0(is_finalized_0, sum_valid_0) {
    return is_finalized_0 && sum_valid_0;
  }
}
...
export const pureCircuits = {
  verify_total: (...args_0) => {
    if (args_0.length !== 2) {
      throw new __compactRuntime.CompactError(`verify_total: expected 2 arguments (as invoked from Typescript), received ${args_0.length}`);
    }
    const is_finalized_0 = args_0[0];
    const sum_valid_0 = args_0[1];
    if (!(typeof(is_finalized_0) === 'boolean')) {
      __compactRuntime.typeError('verify_total',
                                 'argument 1',
                                 'verify_total.compact line 16 char 1',
                                 'Boolean',
                                 is_finalized_0)
    }
    if (!(typeof(sum_valid_0) === 'boolean')) {
      __compactRuntime.typeError('verify_total',
                                 'argument 2',
                                 'verify_total.compact line 16 char 1',
                                 'Boolean',
    ...
    return _dummyContract._verify_total_0(is_finalized_0, sum_valid_0);
  }
};
